...
The gateway supports wallet-signature sessions, registration-first named blobs, native paid registration checks, upload, list, metadata, full downloads, range reads, encrypted storage metadata, and browser CORS. The JavaScript client lives in `../sdk`.

Multipart uploads, an S3-compatible gateway, cross-chain settlement, recovery settlement reserves, and live FCC compute remain explicit follow-up slices.
